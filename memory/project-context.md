...
- No OCR engine is bundled or required yet. This keeps the repo light and SaaS-safe; a future production shape should run OCR in a separate worker/container with per-tenant file isolation and no portal auth material.
- Full verification after bid-state UX plus optional PDF OCR fallback wrapper: `400 passed` for `.\.venv\Scripts\python.exe -m pytest -q -p no:cacheprovider --basetemp pytest-cache-files-full-stage3-wrapper-elevated`.
