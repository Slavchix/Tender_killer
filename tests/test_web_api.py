...
def test_analyze_tender_payload_saves_structured_summary_from_extracted_text(tmp_path):
    ...
    assert payload["analysis"]["operator_view"]["version"] == 3
    assert payload["analysis"]["operator_view"]["decision_brief"]["primary_section"] == "decision_risks"
    assert payload["analysis"]["operator_view"]["metrics"]["documents_ready"] == 1
    detail = get_tender_payload(store.database_path, "mosreg_market", "3668200")
    assert detail["analysis"]["summary"] == payload["analysis"]["summary"]
    assert detail["analysis"]["operator_view"]["version"] == 3
