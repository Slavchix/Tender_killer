...
def test_document_service_skipped_existing_file_recovers_missing_file_status(tmp_path):
    store = TenderStore(tmp_path / "tenders.sqlite")
    store.initialize()
    store.upsert_tender(
        Tender(
    ...
    existing_path = tmp_path / "documents" / "spec.docx"
    ...
    with sqlite3.connect(store.database_path) as connection:
        connection.execute(
            """
            UPDATE tender_documents
            SET local_path = ?, downloaded_at = ?, text_status = ?, text_error = ?
            WHERE source = ? AND external_id = ? AND url = ?
            """,
            (
                str(existing_path),
    ...
    document = payload["document_records"][0]
    assert document["local_path"] == str(existing_path)
    assert document["text_status"] == "downloaded"
    assert document["text_error"] == ""
