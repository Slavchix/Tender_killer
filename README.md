...
Кнопки в меню дублируют основные действия. `Настроить поиск` создает профиль через готовые шаблоны: `Бумага/канцелярия`, `Хозтовары`, `Картриджи/оргтехника`, `Электрика`, `Сантехника`, `Стройматериалы`. Редактирование профиля идет пошагово: выбрать профиль, выбрать поле, затем ввести значение. Быстрые команды остаются как запасной способ.
