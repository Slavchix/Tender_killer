...
1. Continue the auto-pricing pipeline from staged `price_candidates`: expand catalog collectors, persist provider run diagnostics, and keep all discovered prices review-only until operator confirmation.
2. Add auto-stage run orchestration that actively finds prices for all positions, records source/provider diagnostics, and feeds only `auto_eligible` candidates into the existing bulk confirm review action.
